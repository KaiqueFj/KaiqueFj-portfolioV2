...
export default function ProjectsOverview() {
  ...
  return (
    <section className="flex flex-col  mt-24 gap-12">
      ...
      <Reveal variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-[1fr_1fr_0.6fr] gap-8">
        {featuredProjects.map((project) => (
          <Reveal
            className="bg-surface border border-secondary/40 hover:border-accent rounded-2xl  backdrop-blur-md"
            key={project.title}
          >
        ...
        <Reveal className="flex h-full gap-8">
          <Link
            href="/projects"
